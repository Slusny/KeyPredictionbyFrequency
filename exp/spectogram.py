...
print('''How it works?
      ...
      second arg: l for labels, s for saving png ls for labes and saving
      third arg : window size four fourier transform as power of two. (type 3 for 2**3=8 window size)
...
def plot_audio_spectrogram(audio_path, binsize=2**10, plot_path=None, argv = '', colormap="jet"):
    ...
    print("Time bins total      : ", time_bins)
    print("Time per bin         : ",(len(samples)/time_bins+(0.5*binsize))/sample_rate,"s")
    print("Frequency bins total : ", freq_bins)
    print("Frequencies per bin  : ",freq[1],"Hz")
    ...
    #fig = plt.figure(figsize=(time_bins/100, freq_bins/100)) # resolution equal to audio data resolution, dpi=100 as default
    ...
    def format_coord(x, y):
        ...
        if col>=0 and col<numcols and row>=0 and row<numrows:
            z = X[row,col]
            return 'x=%1.4fs, y=%1.4fHz, z=%1.4f'%(x*(len(samples)/time_bins+(0.5*binsize))/sample_rate/2, freq[int(y)], z)
        else:
            return 'x=%1.4f, y=%1.4f'%(x, y)

    ax.format_coord = format_coord
    plt.imshow(X, origin="lower", aspect="auto", cmap=colormap, interpolation="none")
...
if len(sys.argv) == 3:
    ims = plot_audio_spectrogram(sys.argv[1], 2**9, output_folder + '/'+ ntpath.basename(sys.argv[1].replace('.wav','')) + '.png',  sys.argv[2])
if len(sys.argv) == 4:
    ims = plot_audio_spectrogram(sys.argv[1], 2**int(sys.argv[3]), output_folder + '/'+ ntpath.basename(sys.argv[1].replace('.wav','')) + '.png',  sys.argv[2])
else:
    ims = plot_audio_spectrogram(sys.argv[1], 2**10, None, '')

# ims = plot_audio_spectrogram('results/example_appended_5sec.wav', 2**15, output_folder + '/'+ 'example_appended_5sec.png',  'l')